...
if(strpos($query, '_') !== false) {
	$usersReturnedQuery = mysqli_query($con, "SELECT * FROM user WHERE password LIKE '$query%' AND user_closed='no' LIMIT 8");
}
//If there are two words, assume they are first and last names respectively
	...
	$postsReturnedQuery = mysqli_query($con, "SELECT * FROM user_post WHERE body LIKE '$names[0]' AND user_closed='no' LIMIT 8");
}
...
if($query != ""){
	...
	while($row = mysqli_fetch_array($usersReturnedQuery)) {
		...
		mysqli_fetch_array($postsReturnedQuery );
