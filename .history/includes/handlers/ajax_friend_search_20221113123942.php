...
if(strpos($query, "_") !== false) {
	$userReturned = mysqli_query($con, "SELECT * FROM user WHERE username LIKE '$query%' AND user_closed='no' LIMIT 8");
	$postQuery = mysqli_query($con, "SELECT * FROM user_posts WHERE body LIKE '$query%' AND user_closed='no' LIMIT 8");
}
...
if($query != "") {
	...
	while ($prow = mysqli_fetch_array($postQuery))
}
?>
