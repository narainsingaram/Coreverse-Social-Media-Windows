  ...
  return $data;
	...
	public function getConvosDropdown($data, $limit) {
		...
		foreach($convos as $username) {
			...
			$user_found_obj = new `z`($this->con, $username);
			$latest_message_details = $this->getLatestMessage($userLoggedIn, $username);
			...
			$return_string .= "<a href='messages.php?u=$username'> <div class='user_found_messages'>
			<img src='" . $user_found_obj->getProfilePic() . "' style='border-radius: 50px; margin-right: 5px;'>
			" . $user_found_obj->getFirstAndLastName() . "
				<span class='timestamp_smaller id='grey'> " . $latest_message_details[2] . "</span>
				<p id='grey' style='margin:0;'>" . $latest_message_details[0]. $split . " </p>
				</div>
	...
	$return_string .= "<input type='hidden' class='noMoreDropdownData' value='true'> <p style='text-align: center;'> Couldn't find any more friends </p> <a href='messages.php'> <span class='no_friends_found_dropdown'> Message Friends<span> </a> ";
